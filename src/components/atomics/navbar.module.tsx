...
export default function Navbar() {
  ...
  const [authInfo, setAuthInfo] = useState<AuthInfo>(getAuthCookie() || {}); // Inisialisasi dengan hasil dari getAuthCookie atau objek kosong

  useEffect(() => {
    const currentAuth = getAuthCookie();
    setAuthInfo(currentAuth || {});
  }, []); // Efek ini berjalan sekali saat komponen mount untuk mendapatkan auth info
  ...
  useEffect(() => {
    ...
    console.log('Auth Info in useEffect:', authInfo); // Untuk debugging
    console.log('User State in useEffect:', user); // Untuk debugging
  }, [authInfo?.token, authInfo?.userId]); // Dependency pada authInfo?.token dan authInfo?.userId
      ...
      <div className="max-w-7xl mx-auto">
                  ...
                  <div className="absolute right-0 mt-2 w-48 bg-white rounded shadow-md py-2 z-50">
                    ...
                    <button
                      onClick={() => {
                        ...
                        // Perlu diperhatikan: 'role' tidak ada di AuthInfo interface yang kita buat
                        // Jika backend Anda juga menghapus cookie 'role' saat logout,
                        // maka baris di bawah ini perlu disesuaikan.
                        Cookies.remove('role');
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-[#172B4D] shadow-md z-50 py-2 flex flex-col items-center">
            ...
            <Link href="/events" className="block text-white font-medium py-2 hover:underline">Events</Link>
            <Link href="/about" className="block text-white font-medium py-2 hover:underline">About</Link>
            <Link href="/contact" className="block text-white font-medium py-2 hover:underline">Contact</Link>
